...
podman run -d \
    ...
    -e MULTISITE=yes \
    -e SERVER_NAME="${FREEIPA_HOSTNAME} ${KEYCLOAK_HOSTNAME} ${ROUNDCUBE_HOSTNAME} ${NEXTCLOUD_HOSTNAME} ${GLPI_HOSTNAME} ${IRIS_HOSTNAME} ${N8N_HOSTNAME} ${GUACAMOLE_HOSTNAME}" \
    ...
    -e "${N8N_HOSTNAME}_USE_REVERSE_PROXY=yes" \
    -e "${N8N_HOSTNAME}_REVERSE_PROXY_HOST=http://n8n_app:5678" \
    -e "${N8N_HOSTNAME}_SECURITY_MODE=detect" \
    -e "${GUACAMOLE_HOSTNAME}_USE_REVERSE_PROXY=yes" \
    -e "${GUACAMOLE_HOSTNAME}_REVERSE_PROXY_HOST=http://guacamole:8080" \
    -e "${GUACAMOLE_HOSTNAME}_SECURITY_MODE=detect" \
