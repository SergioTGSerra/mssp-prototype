...
while [ $timeout -gt 0 ]; do
    if podman exec -u www-data nextcloud-app php occ status 2>/dev/null | grep -q "installed: true"; then
...
podman exec -u www-data nextcloud-app php occ app:install user_oidc || podman exec -u www-data nextcloud-app php occ app:enable user_oidc
